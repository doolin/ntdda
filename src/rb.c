...
Rb_node
rb_find_key_n(Rb_node n, char *key, int *fnd) {
...
Rb_node
rb_find_key(Rb_node n, char *key) {
...
Rb_node
rb_find_ikey_n(Rb_node n, int ikey, int *fnd) {
...
Rb_node
rb_find_ikey(Rb_node n, int ikey) {
...
Rb_node
rb_find_gkey_n(Rb_node n, char *key,int (*fxn)(char *, char *), int *fnd) {
...
Rb_node
rb_find_gkey(Rb_node n, char *key, int (*fxn)(char *, char *)) {
...
rb_insert_b(Rb_node n, char *key, char *val) {
  ...
  Rb_node newleft, newright, newnode, p; //list;
...
void
rb_print_tree_old(Rb_node t, int level) {
...
void
rb_print_tree(Rb_node t, int level) {
...
void
rb_print_ordered(Rb_node t, int level) {
  ...
  if (ishead(t) && t->p.parent == t) {
    ...
    printf("Head: 0x%x.  Root = 0x%x\n", t, t->p.root);
    rb_print_ordered(t->p.root, 0);
  } else {
    if (isext(t)) {
      //for (i = 0; i < level; i++) putchar(' ');
      //printf("Ext node 0x%x: %c,%c: p=0x%x, k=%s\n",
      //         t, isred(t)?'R':'B', isleft(t)?'l':'r', t->p.parent, t->k.key);
      printf("Ext node, %c, key: %s\n",isred(t)?'R':'B',t->k.key);
    } else {
      rb_print_ordered(t->c.child.left, level+2);
      rb_print_ordered(t->c.child.right, level+2);
      //for (i = 0; i < level; i++) putchar(' ');
#if 0
      printf("Int node 0x%x: %c,%c: l=0x%x, r=0x%x, p=0x%x, lr=(%s,%s)\n",
              t, isred(t)?'R':'B', isleft(t)?'l':'r', t->c.child.left,
...
void
rb_iprint_tree(Rb_node t, int level) {
...
Rb_node
rb_insert_a(Rb_node nd, char *key, char *val) {

  return rb_insert_b(nd->c.list.flink, key, val);
}

Rb_node
rb_insert(Rb_node tree, char *key, void *val) {

  return rb_insert_b(rb_find_key(tree, key), key, val);
}

Rb_node
rb_inserti(Rb_node tree, int ikey, void *val) {

  return rb_insert_b(rb_find_ikey(tree, ikey), (char *) ikey, val);
}

Rb_node
rb_insertg(Rb_node tree, char *key, void *val,
                          int (*func)(char *, char *)) {
